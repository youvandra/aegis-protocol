  ...
  async createRelay(
    ...
    try {
      ...
      if (!senderAddress || !receiverAddress || !amount || amount <= 0) {
        console.error('Invalid relay data:', { senderAddress, receiverAddress, amount });
        return null;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          // relay_number will be auto-generated by the database trigger
        })
        .select()
        .single();

      if (error) {
        console.error('Error creating relay:', error.message, error.details, error.hint);
        return null;
      }
