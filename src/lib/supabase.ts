...
export const streamService = {
  async createGroup(
    ...
    try {
      ...
      // Return with empty members array
      return {
        ...data,
        members: []
      };
  ...
  async getGroups(walletAddress: string): Promise<Group[]> {
    try {
      // Set the wallet address in the request context for RLS
      await supabase.rpc('set_config', {
        setting_name: 'request.jwt.claims',
        setting_value: JSON.stringify({ wallet_address: walletAddress.toLowerCase() })
      });
